...
public class UploadServiceImpl implements UploadService {
	...
	public int fileProcess(FileListVO fileListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		//session 생성
		HttpSession sesseion = httpReq.getSession();
		
		//content domain 생성
		BoardContentDomain boardContentDomain = BoardContentDomain.builder()
				.mbId(sesseion.getAttribute("id").toString())
				...
				//게시글 수정 시 파일관련 물리저장 파일, db 데이터 삭제
				if(fileListVO.getIsEdit() != null) {
					List<BoardFileDomain> fileList = null;
					
					for(MultipartFile multipartFile : multipartFiles) {
						if(!multipartFile.isEmpty()) { //수정 시 새로 파일 첨부될 때 세션에 담긴 파일 지우기
							
							if(sesseion.getAttribute("files") != null) {
								fileList = (List<BoardFileDomain>) sesseion.getAttribute("files");
				...
				for (MultipartFile multipartFile : multipartFiles) {
					if(!multipartFile.isEmpty()) { //파일 있을 떄
						...
						try {
							...
							BoardFileDomain boardFileDomain = BoardFileDomain.builder()
									...
									.upNewFileName("resources/upload/"+newFileName) //WebConfig에 동적 이미지 폴더 생성했기 때문
